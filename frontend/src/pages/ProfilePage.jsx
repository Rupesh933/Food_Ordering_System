    ...
    const handleSubmit = async (e) => {
        ...
        try{
            ...
            if (response.status === 200){
                toast.success(result.message || 'Profile update successfull')
            }
            else{
        ...
        <form onSubmit={handleSubmit} className='card p-4 shadow-sm'>
            <div className='row'>
                ...
                <div className='col-md-6 mb-3'>
                    ...
                    <input type='text' className='form-control' name='reg_date' value={new Date(formData.register_date).toLocaleString()} disabled/>
                </div>
            </div>
            <button type='submit' className='btn btn-primary mt-3'><i className='fas fa-save me-2'></i>Update Profie</button>
        </form>
    </div>
    </PublicLayout>
  )
